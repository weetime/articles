...
const METRICS: M[] = [
  {key: 'hit', label: '前缀缓存命中率', off: 22.0, on: 65.4, unit: '%', decimals: 1, delta: '▲ +197%', up: true},
  {key: 'tps', label: '吞吐量 req/s', off: 2.88, on: 4.73, unit: '', decimals: 2, delta: '▲ +64%', up: true},
...
const StatCard: React.FC<{m: M; idx: number; land: boolean}> = ({m, idx, land}) => {
  ...
  return (
    <div style={{flex: land ? 1 : undefined, background: C.panel, border: `2px solid ${C.line2}`, borderRadius: 22, padding: land ? '24px 24px' : '26px 30px', boxShadow: C.shadow, marginTop: land ? 0 : 22}}>
  ...
  return (
    <Bg>
      <AbsoluteFill style={{opacity: intro, padding: land ? '70px 80px 130px' : '210px 60px 510px', display: 'flex', flexDirection: 'column'}}>
        ...
        <div style={{fontSize: land ? 50 : 58, fontWeight: 800, marginTop: 12, letterSpacing: -1}}>
          一个开关，<span style={{color: C.green}}>四个指标同向改善</span>
        </div>
        ...
        <div style={{fontSize: 22, color: C.ink3, fontWeight: 600, textAlign: 'center'}}>
          命中率↑ → 省掉重复 prefill 的算力 → 吞吐↑ &nbsp;&amp;&nbsp; 延迟↓ · 没有 trade-off
        </div>
      </AbsoluteFill>
    </Bg>
  );
