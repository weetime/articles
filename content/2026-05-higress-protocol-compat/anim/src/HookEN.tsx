    ...
    <Bg>
      <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', padding: land ? '0 80px' : '220px 70px 510px'}}>
        <div style={{fontSize: 27, fontWeight: 800, color: C.violet, letterSpacing: 3, opacity: s1, marginBottom: 26}}>
          HIGRESS · 80 RUNS BENCHMARKED
        </div>
        <div style={{fontSize: land ? 78 : 84, fontWeight: 900, lineHeight: 1.12, textAlign: 'center', letterSpacing: -2, opacity: s1, transform: `translateY(${(1 - s1) * 30}px)`}}>
          One gateway,<br />two protocols<br /><span style={{color: C.cyan}}>side by side</span>
        </div>
        <div style={{marginTop: 44, fontSize: land ? 56 : 64, fontWeight: 900, textAlign: 'center', letterSpacing: -1, opacity: s2, transform: `scale(${qpulse}) translateY(${(1 - s2) * 26}px)`, background: 'linear-gradient(100deg,#7d68ff,#2bc4e6)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent'}}>
          Any conversion cost?
        </div>
