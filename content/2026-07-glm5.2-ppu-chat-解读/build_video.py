...
SHOTS = [
    ("Intro", "GLM 五点二 在八卡 PPU、SGLang 上实测。结论先行：最优配方就是基线，单副本峰值约三百六十九，已到算力上限；抬并发、开投机都是负优化。"),
    ("Recipe", "最优配方长这样：张量并行八，decode 后端 flashmla sparse，并发上限三十二，关投机。下面这些改动我们都试过，实测反而更差。"),
    ("Model", "先看对象：七百四十四 B 的 MoE，激活三十九 B，DSA 稀疏注意力；硬件八张 PPU，Ampere 级、无 FP8。"),
    ("Method", "调优严格控制变量：ShareGPT 负载、关思考，每点三轮取中位，引擎 metrics 归因。"),
    ("ChatWall", "吞吐在并发三十二达峰，三百六十九。再加并发不升反降，首字延迟骤升到十五秒、三十秒——算力已饱和。"),
    ("LowConc", "抬并发上限没用：四十八到九十六，一档都没过三百六十九；这是本配置的算力上限。"),
    ("Mtp", "投机解码也一样：开 EAGLE 单流反而慢一倍，接受率仅十七个点。"),
    ("Roadmap", "若要更高吞吐，几个可能的方向：多副本聚合、PD 分离、W8A8 复评投机。"),
    ("Outro", "以上是要点。完整测试报告——配方、diff、全量数据，都在公众号。下期预告：PD 分离部署，敬请关注。"),
]
...
def display(text):
    # 旁白按 TTS 拼读书写(五点二/八卡…),字幕需还原为规范术语与数字。顺序:先长串后短串。
    for a,b in [("GLM 五点二","GLM-5.2"),
                ("七百四十四 B","744B"),("三百六十九","369"),
                ("三十九 B","39B"),("三十二","32"),("四十八","48"),("六十四","64"),("九十六","96"),
                ("零点六六秒","0.66s"),("十五秒","15s"),("三十秒","30s"),("十七个点","17%"),
                ("张量并行八","张量并行 8"),("八张 PPU","8 张 PPU"),("八卡","8 卡"),
                ("每点三轮","每点 3 轮"),("flashmla sparse","flashmla_sparse")]:
        text=text.replace(a,b)
    # 中日韩字符与数字之间补细空格(数字+拉丁单位如 39B/15s/17% 保持紧凑)
...
def build(orient):
    ...
    fontsize = 44 if land else 46
    marginv = 60 if land else 300   # 字幕落在场景内容(y<1480)与平台 UI(y>1670)之间的安全带
    maxlen = 22 if land else 17
    os.chdir(WORK)
    clips=[]
