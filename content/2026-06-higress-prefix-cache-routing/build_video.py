...
# --en：English Shorts ~60s(hook → hero → topology → barrace → outro),女声 en-US-AvaNeural
EN = "--en" in sys.argv
EN_SHOTS = [
    ("hooken", "Same model, same GPUs — triple the cache hits."),
    ("heroen", "Switch the gateway load balancer from round-robin to prefix-cache. On a 32B model, prefix-cache hits jump from 22 to 65 percent, throughput climbs 64 percent, and first-token latency drops 40 percent."),
    ("topologyen", "Here's why. A cache-blind gateway scatters same-prefix requests across all four pods, so each one re-computes the context. Prefix-cache routing sends them back to the pod that already cached it — a KV hit, with no redundant prefill."),
    ("barraceen", "And it's not a trade-off. Hit rate up 197 percent, throughput up 64 percent, latency down 40 percent, and decode speed up 67 percent — all four improve together, without creating a hotspot."),
    ("outroen", "Bottom line: route by cache, not blindly. On the right workload, this is almost a free lunch. Full breakdown in the description."),
]
if EN:
