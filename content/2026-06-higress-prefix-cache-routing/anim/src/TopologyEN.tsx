...
export const TopologyEN: React.FC = () => {
      ...
      <AbsoluteFill style={{opacity: intro}}>
        <div style={{position: 'absolute', left: land ? 80 : 60, top: land ? 60 : 220}}>
          ...
          <div style={{fontSize: land ? 50 : 56, fontWeight: 800, marginTop: 12, letterSpacing: -1}}>
            Same prefix <span style={{color: isOn ? C.green : C.off}}>{isOn ? '→ same pod, cache hit' : 'scattered at random'}</span>
          </div>
        </div>
        ...
        <Node x={g.cli.x} y={g.cli.y} w={g.cliW} h={g.cliH}>
          <div style={{fontSize: 28, fontWeight: 800}}>Clients / Agents</div>
          <div style={{fontSize: 18, color: C.ink3, marginTop: 6}}>many requests · shared long prefix</div>
        </Node>

        <Node x={g.gw.x} y={g.gw.y} w={g.gwW} h={g.gwH} glow={C.violet} glowI={isOn ? 0.85 : 0} border={isOn ? C.violet + '66' : C.line2}>
          <div style={{fontSize: 28, fontWeight: 800}}>Higress gateway</div>
          <div style={{fontSize: 18, color: isOn ? C.violet : C.ink3, marginTop: 6, fontWeight: 700}}>{isOn ? 'KV-aware · longest-prefix match' : 'cache-blind · round-robin'}</div>
        </Node>
          ...
          return (
            <Node key={i} x={pc.x} y={pc.y} w={g.podW} h={g.podH} glow={glowColor} glowI={gg} border={border}>
              ...
              <div style={{marginTop: 10, fontSize: 17, fontWeight: 800, padding: '5px 12px', borderRadius: 9, color: gg > 0.05 ? (on ? C.green : C.ink3) : C.ink4, background: gg > 0.05 ? (on ? `${C.green}22` : `${C.off}22`) : 'transparent', opacity: gg > 0.05 ? 0.5 + gg * 0.5 : 0.45}}>
                {gg > 0.05 ? (on ? '✓ KV hit' : '✗ re-prefill') : on ? 'hit' : 'idle'}
              </div>
            </Node>
          );
        })}
        ...
        <div style={{...meterStyle, background: C.panel, border: `2px solid ${C.line2}`, borderRadius: 22, padding: '28px 34px', boxShadow: C.shadow}}>
          <div style={{display: 'flex', alignItems: 'baseline'}}>
            <div style={{fontSize: 24, fontWeight: 700, color: C.ink2}}>Prefix-cache hit rate</div>
          ...
          <div style={{marginTop: 12, fontSize: 19, color: C.ink3, fontWeight: 600}}>
            {isOn ? 'Same prefix hits → skip the redundant prefill' : 'Prefix scattered → every pod re-computes it'}
          </div>
        </div>

        {trAlpha > 0.01 && (
          <AbsoluteFill style={{alignItems: 'center', justifyContent: 'center', opacity: trAlpha}}>
            <div style={{background: 'linear-gradient(100deg,#6d4bff,#0891b2)', color: '#fff', fontSize: 44, fontWeight: 850, padding: '28px 50px', borderRadius: 22, boxShadow: '0 20px 60px #6d4bff55', transform: `translateX(${interpolate(trSweep, [0, 1], [-40, 40])}px)`}}>
              switch to prefix_cache → KV-aware
            </div>
          </AbsoluteFill>
        )}
      </AbsoluteFill>
