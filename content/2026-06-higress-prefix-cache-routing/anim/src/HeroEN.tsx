...
const Pill: React.FC<{label: string; val: string; color: string; delay: number}> = ({label, val, color, delay}) => {
  ...
  return (
    <div style={{flex: 1, background: C.panel, border: `2px solid ${C.line2}`, borderRadius: 18, padding: '22px 6px', textAlign: 'center', boxShadow: C.shadow, opacity: s, transform: `translateY(${(1 - s) * 22}px)`}}>
...
const Chip: React.FC<{children: React.ReactNode; delay: number}> = ({children, delay}) => {
  ...
  return (
    <div style={{display: 'flex', alignItems: 'center', gap: 9, background: C.panel, border: `1px solid ${C.line2}`, borderRadius: 14, padding: '14px 20px', fontSize: 19, fontWeight: 600, color: C.ink2, opacity: o, transform: `translateY(${(1 - o) * 10}px)`, boxShadow: C.shadow}}>
...
export const HeroEN: React.FC = () => {
  ...
  const titleBlock = (
    <div>
      <div style={{display: 'flex', alignItems: 'center', gap: 12, fontSize: 23, fontWeight: 700, color: C.violet, opacity: t1}}>
        ...
        ModelDoctor · Routing-strategy benchmark
      </div>
      <div style={{fontSize: land ? 70 : 76, fontWeight: 850, lineHeight: 1.06, letterSpacing: -2, marginTop: 20, opacity: t1, transform: `translateY(${(1 - t1) * 18}px)`}}>
        One routing switch
      </div>
      <div style={{fontSize: land ? 52 : 56, fontWeight: 850, letterSpacing: -1.5, marginTop: 8, opacity: t2, transform: `translateY(${(1 - t2) * 18}px)`, background: 'linear-gradient(100deg,#7d68ff,#2bc4e6)', WebkitBackgroundClip: 'text', backgroundClip: 'text', color: 'transparent'}}>
        3× cache hits · +64% throughput
      </div>
    </div>
  );
    ...
    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
      <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 20}}>
        <div style={{textAlign: 'center'}}>
          <div style={{fontSize: land ? 100 : 116, fontWeight: 850, color: C.off, letterSpacing: -4}}>22%</div>
          <div style={{fontSize: 20, color: C.ink3, fontWeight: 600}}>cache-blind · round-robin</div>
        </div>
        ...
        <div style={{textAlign: 'center'}}>
          ...
          <div style={{fontSize: 20, color: C.violet, fontWeight: 700}}>prefix_cache · KV-aware</div>
        </div>
      </div>
      <span style={{marginTop: 24, fontSize: 27, fontWeight: 800, color: C.green, background: `${C.green}1f`, border: `1px solid ${C.green}55`, borderRadius: 999, padding: '11px 26px', opacity: badge, transform: `scale(${0.8 + badge * 0.2})`}}>
        Prefix-cache hit rate ▲ +197%
      </span>
    </div>
  );
  ...
  const pillsRow = (
    <div style={{display: 'flex', gap: 14}}>
      <Pill label="Throughput" val="+64%" color={C.green} delay={96} />
  ...
  const chipsRow = (
    <div style={{display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: land ? 'flex-start' : 'center'}}>
      <Chip delay={132}>Workload <b style={{color: C.ink}}>RAG/Agent long-context</b></Chip>
      <Chip delay={142}><b style={{color: C.ink}}>4 Pods</b> · Qwen3-32B</Chip>
      <Chip delay={152}>OFF/ON · <b style={{color: C.ink}}>5 runs × 480 reqs</b></Chip>
    </div>
  );
